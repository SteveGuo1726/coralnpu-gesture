...
build_dir="${TMPDIR:-/tmp}/gestureflow_conv4x4_cin_same_stream_dmp_pointwise_out32_verilator"
...
VERILATOR_ROOT="$verilator_root" verilator --binary --timing --sv \
  --top-module tb_gestureflow_conv4x4_cin_same_stream_dmp_pointwise_out32 \
  --Mdir "$build_dir" -I"$root/tests" \
  ...
  "$root/tests/tb_gestureflow_conv4x4_cin_same_stream_dmp_pointwise_out32.sv"
timeout 300s "$build_dir/Vtb_gestureflow_conv4x4_cin_same_stream_dmp_pointwise_out32"
