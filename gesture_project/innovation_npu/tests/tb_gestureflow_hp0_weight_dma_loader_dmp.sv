...
`timescale 1ns/1ps
module tb_gestureflow_hp0_weight_dma_loader_dmp #(parameter int TEST_OUTPUTS = 16);
  ...
  logic [31:0] byte_count = 32'(4 * (TEST_OUTPUTS/2) * TEST_TAPS * TEST_GROUPS * 6);
  ...
  logic [$clog2(TEST_OUTPUTS/2)-1:0] weight_write_pair;
  ...
  gestureflow_hp0_weight_dma_loader_dmp #(.FIFO_BEATS(16), .MAX_OUTPUT_LANES(TEST_OUTPUTS)) dut (
    ...
    .bytes_read, .write_count, .weight_write_valid, .weight_write_pair,
  ...
  always_ff @(posedge clk) begin
    if (rst_n && weight_write_valid) begin
      for (int wi = 0; wi < 6; wi++) begin
        if (weight_write_data[wi*32 +: 32] !== (32'hA0000000 + expected_word + wi))
          $fatal(1, "bad packed word=%0d wi=%0d got=%08x", expected_word, wi, weight_write_data[wi*32 +: 32]);
      end
      if (int'(weight_write_pair) !== (expected_word / (TEST_TAPS * TEST_GROUPS * 6)) ||
          int'(weight_write_tap) !== ((expected_word / (TEST_GROUPS * 6)) % TEST_TAPS) ||
          int'(weight_write_ic_group) !== ((expected_word / 6) % TEST_GROUPS))
        $fatal(1, "bad coordinate word=%0d pair=%0d tap=%0d group=%0d", expected_word,
               weight_write_pair, weight_write_tap, weight_write_ic_group);
      expected_word = expected_word + 6;
  ...
  initial begin
    ...
    for (integer cycle = 0; cycle < 2000; cycle = cycle + 1) begin
      ...
      if (done) begin
        if (expected_writes != (TEST_OUTPUTS/2) * TEST_TAPS * TEST_GROUPS ||
            bytes_read != byte_count || write_count != (TEST_OUTPUTS/2) * TEST_TAPS * TEST_GROUPS)
