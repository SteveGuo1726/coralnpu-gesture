...
# Engine-level 32-input-channel first-tile regression for the HaGRID-18
# conv3 layer (32->32, 8 input-channel groups).
root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
build="${TMPDIR:-/tmp}/gestureflow_conv4x4_cin32_tile0_verilator"
rm -rf "$build"
v="$(readlink -f "$(command -v verilator)")"
VERILATOR_ROOT="$(cd "$(dirname "$v")/.." && pwd)" verilator --binary --timing --sv \
  --top-module tb_gestureflow_conv4x4_cin32_tile0_real --Mdir "$build" -I"$root/tests" \
  ...
  "$root/tests/tb_gestureflow_conv4x4_cin32_tile0_real.sv"
timeout 180s "$build/Vtb_gestureflow_conv4x4_cin32_tile0_real"
