...
# Relay regression for the first two layers. The HP0 read channel must stay
# silent during layer1, proving the output-bank relay path is active.
root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
build="${TMPDIR:-/tmp}/gestureflow_layer_chain_relay_axil_verilator"
VERILATOR_ROOT=/home/steveguo/verilator /home/steveguo/verilator/bin/verilator_bin --binary --timing --sv \
  --top-module tb_gestureflow_layer_chain_relay_axil --Mdir "$build" -I"$root/tests" \
  ...
  "$root/tests/tb_gestureflow_layer_chain_relay_axil.sv"
timeout 180s "$build/Vtb_gestureflow_layer_chain_relay_axil"
