...
VERILATOR_ROOT="$(cd "$(dirname "$v")/.." && pwd)" verilator --binary --timing --sv \
  --top-module tb_gestureflow_conv4x4_cin32_all32_real --Mdir "$build" -I"$root/tests" \
  ...
  "$root/tests/tb_gestureflow_conv4x4_cin32_all32_real.sv"
timeout 180s "$build/Vtb_gestureflow_conv4x4_cin32_all32_real"
