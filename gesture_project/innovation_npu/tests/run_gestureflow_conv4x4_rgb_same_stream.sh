...
build_dir="${TMPDIR:-/tmp}/gestureflow_conv4x4_rgb_same_stream_verilator"
...
VERILATOR_ROOT="$(cd "$(dirname "$verilator_bin")/.." && pwd)" verilator --binary --timing --sv \
  ...
  "$root/rtl/gestureflow_conv4x4_rgb_same_stream.sv" \
  "$root/tests/tb_gestureflow_conv4x4_rgb_same_stream.sv"
"$build_dir/Vtb_gestureflow_conv4x4_rgb_same_stream"
