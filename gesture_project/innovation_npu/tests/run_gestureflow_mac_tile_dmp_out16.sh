...
build_dir="${TMPDIR:-/tmp}/gestureflow_mac_tile_dmp_out16_verilator"
...
VERILATOR_ROOT="$verilator_root" verilator --binary --timing --sv \
  --top-module tb_gestureflow_mac_tile_dmp_out16 \
  --Mdir "$build_dir" \
  "$root/rtl/gestureflow_weight_bank.sv" \
  "$root/rtl/gestureflow_mac_tile_dmp.sv" \
  "$root/tests/tb_gestureflow_mac_tile_dmp_out16.sv"
"$build_dir/Vtb_gestureflow_mac_tile_dmp_out16"
