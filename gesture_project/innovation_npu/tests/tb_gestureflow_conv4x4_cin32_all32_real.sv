...
`timescale 1ns/1ps
module tb_gestureflow_conv4x4_cin32_all32_real;
  `include "generated_gestureflow_real_conv4x4_cin32_layer.svh"

  localparam int GF_TILE_LANES = 32;
  ...
  gestureflow_conv4x4_cin_same_stream #(
    ...
    .weight_write_data(weight_write_data), .weight_bank_select(weight_bank_select), .read_bank_select(1'b0),
        ...
        if ((row == int'(gf_cin32_probe_y[probe])) && (column == int'(gf_cin32_probe_x[probe]))) begin
          for (int lane = 0; lane < GF_TILE_LANES; lane++) begin
            if ($signed(quant_data[lane]) !== gf_cin32_probe_quant[probe][lane]) begin
              $fatal(1, "cin32 all32 probe mismatch y=%0d x=%0d lane=%0d got=%0d expected=%0d",
  ...
  initial begin
    ...
    if (raw_fault || quant_fault || !frame_input_done_seen || probe_hits != GF_CIN32_PROBE_COUNT ||
      ...
      $fatal(1, "cin32 all32 failed outputs=%0d probes=%0d done=%b raw_fault=%b quant_fault=%b hash=%08x",
        output_count, probe_hits, frame_input_done_seen, raw_fault, quant_fault, output_hash);
    end
    $display("GESTUREFLOW_CONV4X4_CIN32_ALL32_REAL_PASS outputs=%0d probes=%0d hash=%08x",
