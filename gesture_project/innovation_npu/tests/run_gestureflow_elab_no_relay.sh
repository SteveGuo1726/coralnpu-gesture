...
# Elaboration-only regression for the board build (ENABLE_RELAY=0). Catches
# missing tie-offs and multiple-driver errors in the single output-bank path
# without a slow Vivado run.
root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
build="${TMPDIR:-/tmp}/gestureflow_elab_no_relay_verilator"
rm -rf "$build"
VERILATOR_ROOT=/home/steveguo/verilator /home/steveguo/verilator/bin/verilator_bin --binary --timing --sv \
  --top-module tb_gestureflow_elab_no_relay --Mdir "$build" -I"$root/tests" \
  ...
  "$root/tests/tb_gestureflow_elab_no_relay.sv"
timeout 60s "$build/Vtb_gestureflow_elab_no_relay"
