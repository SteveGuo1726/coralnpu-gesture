...
// DMP pointwise full-layer regression for the HaGRID-18 1x1 head first tile
// (48x12x12 -> 32x12x12).  The engine walks six eight-channel groups with one
// active tap and 16 output pairs.
module tb_gestureflow_conv4x4_cin_same_stream_dmp_pointwise_out32;
  `include "generated_gestureflow_dmp_head1x1_t0_golden.svh"

  localparam int W = GF_DMP_HEAD1X1_T0_WIDTH;
  localparam int H = GF_DMP_HEAD1X1_T0_HEIGHT;
  localparam int C = GF_DMP_HEAD1X1_T0_INPUT_CHANNELS;
  localparam int OC = GF_DMP_HEAD1X1_T0_OUTPUT_LANES;
  localparam int PAIRS = OC / 2;
  localparam int GROUPS = GF_DMP_HEAD1X1_T0_INPUT_GROUPS;
  ...
  always @(posedge clk) begin
    ...
    if (output_valid && output_ready) begin
      ...
      for (int probe = 0; probe < GF_DMP_HEAD1X1_T0_PROBE_COUNT; probe++) begin
        if ((row == int'(gf_dmp_head1x1_t0_probe_y[probe])) &&
            (col == int'(gf_dmp_head1x1_t0_probe_x[probe]))) begin
          for (int lane = 0; lane < OC; lane++) begin
            if (output_psum[lane] !== gf_dmp_head1x1_t0_probe_raw[probe][lane]) begin
              $fatal(1, "DMP pointwise out32 probe mismatch y=%0d x=%0d lane=%0d got=%0d expected=%0d",
                     row, col, lane, output_psum[lane], gf_dmp_head1x1_t0_probe_raw[probe][lane]);
            end
          end
  ...
  task automatic load_weight_entry(input int pair, input int group);
    logic [8*24-1:0] wdata;
    begin
      for (int word = 0; word < WORDS_PER_ENTRY; word++) begin
        int base = ((pair * GROUPS) + group) * WORDS_PER_ENTRY + word;
        wdata[word*32 +: 32] = gf_dmp_head1x1_t0_weights_dma[base];
      end
      @(negedge clk);
  ...
  initial begin
    for (int ch = 0; ch < C; ch++) begin
      input_zero_point[ch] = GF_DMP_HEAD1X1_T0_INPUT_ZERO_POINT[7:0];
    end
    for (int lane = 0; lane < OC; lane++) begin
      bias[lane] = gf_dmp_head1x1_t0_folded_bias[lane];
    end
    ...
    for (int row = 0; row < H; row++) begin
      for (int col = 0; col < W; col++) begin
        ...
        for (int ch = 0; ch < C; ch++) begin
          pixel_data[ch] = gf_dmp_head1x1_t0_input_q[((row * W) + col) * C + ch];
        end
        pixel_valid = 1'b1;
        @(negedge clk);
        pixel_valid = 1'b0;
    ...
    for (int watchdog = 0; watchdog < 2500000 && output_count < H*W; watchdog++) begin
      @(negedge clk);
    end
    @(negedge clk);
    if (protocol_error || !frame_input_done_seen || probe_hits != GF_DMP_HEAD1X1_T0_PROBE_COUNT ||
        output_count != H*W || output_hash != GF_DMP_HEAD1X1_T0_RAW_FNV1A) begin
      $fatal(1, "DMP pointwise out32 failed outputs=%0d probes=%0d done=%b fault=%b hash=%08x expected=%08x",
             output_count, probe_hits, frame_input_done_seen, protocol_error,
             output_hash, GF_DMP_HEAD1X1_T0_RAW_FNV1A);
    end
    $display("GESTUREFLOW_CONV4X4_CIN_SAME_STREAM_DMP_POINTWISE_OUT32_PASS outputs=%0d probes=%0d raw_fnv1a=%08x",
             output_count, probe_hits, output_hash);
    $display("GESTUREFLOW_CONV4X4_CIN_SAME_STREAM_DMP_POINTWISE_OUT32_CYCLES=%0d",
