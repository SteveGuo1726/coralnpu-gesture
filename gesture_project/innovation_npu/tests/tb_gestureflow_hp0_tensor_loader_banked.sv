...
`timescale 1ns/1ps
module tb_gestureflow_hp0_tensor_loader_banked #(
  parameter int CHANNELS = 80
);
  localparam int PIXELS = 3;
  localparam int TOTAL_BYTES = CHANNELS * PIXELS;
  logic clk = 0;
  always #5 clk = ~clk;
  logic rst_n = 0, start = 0, clear = 0, pixel_ready = 1;
  logic [31:0] source_addr = 32'h00001000, byte_count = TOTAL_BYTES;
  logic [13:0] pixel_count = 14'(PIXELS);
  logic busy, done, fault, frame_start, pixel_valid;
  logic signed [CHANNELS-1:0][7:0] pixel_data;
  ...
  function automatic logic signed [7:0] source_byte(input integer offset);
    source_byte = 8'(offset - (TOTAL_BYTES / 2));
  endfunction
  ...
  function automatic logic [63:0] response_word(input logic [31:0] addr);
    ...
    begin
      ...
      for (lane = 0; lane < 8; lane++)
        word[lane*8 +: 8] = source_byte(int'(addr) - 32'h1000 + lane);
      response_word = word;
    end
  endfunction
  ...
  gestureflow_hp0_tensor_loader_banked #(.CHANNELS(CHANNELS)) dut (
    ...
    if (!rst_n) begin
      ...
      if (pixel_valid && pixel_ready) begin
        for (int channel = 0; channel < CHANNELS; channel++) begin
          if (pixel_data[channel] !== source_byte(observed_pixels * CHANNELS + channel))
            $fatal(1, "lane mismatch channels=%0d pixel=%0d channel=%0d expected=%0d got=%0d",
              CHANNELS, observed_pixels, channel,
              source_byte(observed_pixels * CHANNELS + channel), pixel_data[channel]);
        end
  ...
  initial begin
    ...
    repeat (1600) begin
      @(posedge clk);
      if (done) begin
        if (fault || bytes_read != TOTAL_BYTES || pixels_emitted != 14'(PIXELS) ||
            observed_pixels != PIXELS || observed_frame_starts != 1)
          $fatal(1, "banked loader summary channels=%0d fault=%0b bytes=%0d pixels=%0d observed=%0d frames=%0d",
            CHANNELS, fault, bytes_read, pixels_emitted, observed_pixels, observed_frame_starts);
        $display("GESTUREFLOW_HP0_TENSOR_LOADER_BANKED_PASS channels=%0d bytes=%0d pixels=%0d",
          CHANNELS, bytes_read, pixels_emitted);
        $finish;
      end
    end
    $fatal(1, "banked loader timeout channels=%0d busy=%0b fault=%0b bytes=%0d pixels=%0d",
      CHANNELS, busy, fault, bytes_read, pixels_emitted);
  end
endmodule
