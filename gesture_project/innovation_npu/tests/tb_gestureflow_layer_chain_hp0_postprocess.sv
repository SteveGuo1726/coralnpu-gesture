...
// Top-level mode-4 regression for the project-local HP0 GAP/FC path.  The
// memory model returns a zero 12x12x112 tensor and checks that the shared
// layer-chain read mux actually forwards bursts from the postprocess loader.
`timescale 1ns/1ps
module tb_gestureflow_layer_chain_hp0_postprocess;
  ...
  localparam logic [31:0] POST_PROGRESS = 32'h09c;
  localparam logic [31:0] SOURCE_BASE = 32'h00001000;
  localparam int INPUT_BYTES = 144 * 112;
  ...
  initial begin
    ...
    for (int wait_cycle = 0; wait_cycle < 200000; wait_cycle++) begin
      @(negedge clk);
      if (dut.fault) $fatal(1, "mode4 top fault progress=%08x", dut.post_gap_values_done);
      if (dut.done) begin
        if (accepted_read_bursts == 0 || dut.post_gap_values_done != 112 ||
            dut.post_fc_values_done != 6 || dut.post_predicted_class != 0)
          $fatal(1, "mode4 completion invalid bursts=%0d gap=%0d fc=%0d class=%0d",
            accepted_read_bursts, dut.post_gap_values_done, dut.post_fc_values_done,
            dut.post_predicted_class);
        $display("GESTUREFLOW_LAYER_CHAIN_HP0_POSTPROCESS_PASS bursts=%0d cycles=%0d progress=%08x",
          accepted_read_bursts, dut.post_cycles, {19'd0, dut.post_fc_values_done,
          dut.post_gap_values_done, dut.post_busy, dut.post_fault, dut.post_done});
        $finish;
      end
    end
    $fatal(1, "mode4 top timeout progress=%08x bursts=%0d bytes=%0d",
      {19'd0, dut.post_fc_values_done, dut.post_gap_values_done,
       dut.post_busy, dut.post_fault, dut.post_done},
