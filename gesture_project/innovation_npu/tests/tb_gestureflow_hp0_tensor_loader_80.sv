...
// Mode 3 must not expose a partial 80-byte NHWC pixel. This bounded reader
// regression verifies ten AXI beats assemble atomically before the rolling
// window may consume one pixel.
`timescale 1ns/1ps
module tb_gestureflow_hp0_tensor_loader_80;
  logic clk = 0;
  always #5 clk = ~clk;
  logic rst_n = 0, start = 0, clear = 0, pixel_ready = 1;
  logic [31:0] source_addr = 32'h00001000, byte_count = 240;
  logic [13:0] pixel_count = 3;
  logic busy, done, fault, frame_start, pixel_valid;
  logic signed [79:0][7:0] pixel_data;
  ...
  gestureflow_hp0_tensor_loader #(.CHANNELS(80)) dut (
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      if (pixel_valid && pixel_ready) begin
        for (int channel = 0; channel < 80; channel++) begin
          if (pixel_data[channel] !== source_byte(observed_pixels * 80 + channel)) begin
            $fatal(1, "lane mismatch pixel=%0d channel=%0d expected=%0d got=%0d", observed_pixels, channel,
              source_byte(observed_pixels * 80 + channel), pixel_data[channel]);
          end
        end
  ...
  initial begin
    ...
    repeat (900) begin
      @(posedge clk);
      if (done) begin
        if (fault || bytes_read != 240 || pixels_emitted != 3 || observed_pixels != 3 || observed_frame_starts != 1)
        ...
        $display("GESTUREFLOW_HP0_TENSOR_LOADER_80_PASS bytes=%0d pixels=%0d", bytes_read, pixels_emitted);
        $finish;
      end
    end
    $fatal(1, "80-channel loader timeout busy=%0b fault=%0b bytes=%0d pixels=%0d", busy, fault, bytes_read, pixels_emitted);
  end
endmodule
