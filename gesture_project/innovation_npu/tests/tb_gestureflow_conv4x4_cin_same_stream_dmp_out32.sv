...
// 32-output DMP full-layer regression using the real HaGRID-18 conv2a layer
// (16x48x48 -> 32x48x48).  It proves the DMP engine's 16-pair/8-input-lane
// parameterization and its INT32 accumulation across two eight-channel input
// groups before the full network is synthesized.
module tb_gestureflow_conv4x4_cin_same_stream_dmp_out32;
  `include "generated_gestureflow_dmp_conv2a_golden.svh"

  localparam int W = GF_DMP_CONV2A_WIDTH;
  localparam int H = GF_DMP_CONV2A_HEIGHT;
  localparam int C = GF_DMP_CONV2A_INPUT_CHANNELS;
  localparam int OC = GF_DMP_CONV2A_OUTPUT_LANES;
  localparam int PAIRS = OC / 2;
  localparam int GROUPS = GF_DMP_CONV2A_INPUT_GROUPS;
  ...
  always @(posedge clk) begin
    ...
    if (output_valid && output_ready) begin
      ...
      for (int probe = 0; probe < GF_DMP_CONV2A_PROBE_COUNT; probe++) begin
        if ((row == int'(gf_dmp_conv2a_probe_y[probe])) &&
            (col == int'(gf_dmp_conv2a_probe_x[probe]))) begin
          for (int lane = 0; lane < OC; lane++) begin
            if (output_psum[lane] !== gf_dmp_conv2a_probe_raw[probe][lane]) begin
              $fatal(1, "DMP out32 probe mismatch y=%0d x=%0d lane=%0d got=%0d expected=%0d",
                     row, col, lane, output_psum[lane], gf_dmp_conv2a_probe_raw[probe][lane]);
            end
          end
  ...
  task automatic load_weight_entry(input int pair, input int tap, input int group);
    ...
    begin
      for (int word = 0; word < WORDS_PER_ENTRY; word++) begin
        int base = (((pair * GF_DMP_CONV2A_ACTIVE_TAPS) + tap) * GROUPS + group) * WORDS_PER_ENTRY + word;
        wdata[word*32 +: 32] = gf_dmp_conv2a_weights_dma[base];
      end
      @(negedge clk);
  ...
  initial begin
    for (int ch = 0; ch < C; ch++) begin
      input_zero_point[ch] = GF_DMP_CONV2A_INPUT_ZERO_POINT[7:0];
    end
    for (int lane = 0; lane < OC; lane++) begin
      bias[lane] = gf_dmp_conv2a_folded_bias[lane];
    end
    repeat (3) @(negedge clk);
    rst_n = 1'b1;

    for (int pair = 0; pair < PAIRS; pair++) begin
      for (int tap = 0; tap < GF_DMP_CONV2A_ACTIVE_TAPS; tap++) begin
    ...
    for (int row = 0; row < H; row++) begin
      for (int col = 0; col < W; col++) begin
        ...
        for (int ch = 0; ch < C; ch++) begin
          pixel_data[ch] = gf_dmp_conv2a_input_q[((row * W) + col) * C + ch];
        end
        pixel_valid = 1'b1;
        @(negedge clk);
        pixel_valid = 1'b0;
    ...
    @(negedge clk);
    if (protocol_error || !frame_input_done_seen || probe_hits != GF_DMP_CONV2A_PROBE_COUNT ||
        output_count != H*W || output_hash != GF_DMP_CONV2A_RAW_FNV1A) begin
      $fatal(1, "DMP out32 layer failed outputs=%0d probes=%0d done=%b fault=%b hash=%08x expected=%08x",
             output_count, probe_hits, frame_input_done_seen, protocol_error,
             output_hash, GF_DMP_CONV2A_RAW_FNV1A);
    end
    $display("GESTUREFLOW_CONV4X4_CIN_SAME_STREAM_DMP_OUT32_PASS outputs=%0d probes=%0d raw_fnv1a=%08x",
             output_count, probe_hits, output_hash);
    $display("GESTUREFLOW_CONV4X4_CIN_SAME_STREAM_DMP_OUT32_CYCLES=%0d",
