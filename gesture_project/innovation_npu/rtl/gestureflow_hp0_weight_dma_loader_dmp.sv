...
`timescale 1ns/1ps
module gestureflow_hp0_weight_dma_loader_dmp #(
  ...
  output logic [((MAX_OUTPUT_LANES/2) <= 1 ? 1 : $clog2(MAX_OUTPUT_LANES/2))-1:0] weight_write_pair,
  output logic [3:0] weight_write_tap,
  output logic [4:0] weight_write_ic_group,
  output logic [8*24-1:0] weight_write_data,
  ...
  localparam int OC_W = (MAX_OUTPUT_LANES/2 <= 1) ? 1 : $clog2(MAX_OUTPUT_LANES/2);
  state_t state;
  ...
  logic [OC_W-1:0] pair_count;
  logic [3:0] tap_count;
  logic [4:0] group_count;
  logic word_half;
  logic [2:0] word_in_pack;
  logic [31:0] current_word;
  logic [8*24-1:0] pack_accum;
  logic [8*24-1:0] packed_word;
  logic current_word_valid;
  logic read_fire, word_fire, pack_write_valid, beat_release;
  ...
  assign word_fire = (state != IDLE) && current_word_valid && (words_remaining != 0);
  assign pack_write_valid = word_fire && (word_in_pack == 3'd5);
  assign weight_write_valid = pack_write_valid;
  assign beat_release = word_fire && word_half;
  ...
  always_comb begin
    ...
    packed_word = pack_accum;
    packed_word[word_in_pack*32 +: 32] = current_word;
    weight_write_data = packed_word;
    weight_write_pair = pair_count;
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      cfg_outputs <= '0; cfg_pairs <= '0; cfg_source_addr <= '0; cfg_byte_count <= '0;
      cfg_output_taps <= '0; cfg_expected_words <= '0;
      pair_count <= '0; tap_count <= '0; group_count <= '0; word_half <= 1'b0;
      word_in_pack <= '0; pack_accum <= '0;
      ...
      if (word_fire) begin
        ...
        pack_accum <= packed_word;
        if (word_in_pack == 3'd5) begin
          word_in_pack <= '0;
          write_count <= write_count + 1'b1;
        end else begin
          word_in_pack <= word_in_pack + 1'b1;
        end
        if (!word_half) begin
          word_half <= 1'b1;
        end else begin
        ...
        if (pack_write_valid) begin
          if (group_count == cfg_groups - 1'b1) begin
            group_count <= '0;
            if (tap_count == (cfg_taps[3:0] - 4'd1)) begin
              tap_count <= '0;
              if (pair_count == OC_W'(cfg_pairs - 1'b1)) pair_count <= '0;
              else pair_count <= pair_count + 1'b1;
      ...
      case (state)
        ...
        CONFIG: begin
          ...
          if ((cfg_source_addr[2:0] != 0) || (cfg_byte_count == 0) ||
            ...
            cfg_output_taps <= cfg_pairs * cfg_taps;
            state <= CONFIG_PRODUCT;
          end
        end
        CONFIG_PRODUCT: begin
          cfg_expected_words <= cfg_output_taps * cfg_groups * 6;
          state <= VALIDATE;
        end
        VALIDATE: begin
