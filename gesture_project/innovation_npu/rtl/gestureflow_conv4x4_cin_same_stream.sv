...
//
// Real body-convolution engine for NHWC INT8 tensors. A physical 16-tap window
// is held locally while the existing 16-output x 4-input-lane MAC tile walks
// all input-channel groups; therefore INT32 partial sums never leave the tile.
...
module gestureflow_conv4x4_cin_same_stream #(
  ...
  // PROJECT_LOCAL_SELF_RESEARCH_NOT_GOOGLE_OFFICIAL
  // Runtime pointwise mode reuses this same shared MAC backend for 1x1.
  ...
  always_comb begin
    ...
    for (int lane = 0; lane < 4; lane++) begin
      if (pointwise_mode) tile_activation[lane] = held_pixel[int'(ic_group_index) * 4 + lane];
      else tile_activation[lane] = held_window[int'(ic_group_index) * 4 + lane][tap_index];
    end
    output_row = held_row;
  ...
  gestureflow_mac_tile #(
    ...
    .mac_tap(pointwise_mode ? 4'd0 : tap_index), .mac_ic_group(ic_group_index), .activation(tile_activation),
    ...
    if (!rst_n) begin
      ...
      if (mac_active && tile_mac_ready) begin
        if (ic_group_index == IC_GROUP_W'(input_group_count - 1'b1)) begin
          ic_group_index <= '0;
          if (pointwise_mode || (tap_index == 4'(ACTIVE_TAPS - 1))) mac_active <= 1'b0;
