...
# Keep the descriptor build physically separate from the signed writer-burst
# baseline. Vivado may continue an implementation child after a WSL timeout;
# sharing the directory would let that child corrupt the rollback project.
...
cp -f "$board/build_gestureflow_layer_chain_hp0_7020.tcl" "$dst/project_local_gestureflow/"
cmd.exe /d /s /c "pushd E:\coralnpu_vivado\projects\gestureflow_layer_chain_descriptor_hp0_7020_v1\project_local_gestureflow && call E:\Xilinx\Vivado\2023.2\bin\vivado.bat -mode batch -source build_gestureflow_layer_chain_hp0_7020.tcl"
