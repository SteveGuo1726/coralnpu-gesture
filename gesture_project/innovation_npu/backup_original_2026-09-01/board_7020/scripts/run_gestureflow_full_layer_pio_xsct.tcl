...
set project_root "E:/coralnpu_vivado/projects/gestureflow_full_layer_pio_7020_v1"
set bit_path [file join $project_root axi_gpio.runs impl_1 system_wrapper.bit]
set xsa_path [file join $project_root logs gestureflow_full_pio_7020.xsa]
set ps7_init_path [file join $project_root axi_gpio.srcs sources_1 bd system ip system_processing_system7_0_0 ps7_init.tcl]
set elf_path [file join $project_root vitis axi_gpio Debug gestureflow_full_layer_pio.elf]
set probe_base 0xFFFF0000

connect -url tcp:127.0.0.1:3333
...
puts [format {GESTUREFLOW_FULL_LAYER_PIO_FINAL_RESULT = 0x%08X} $final]
for {set i 0} {$i < 13} {incr i} {
  puts [format {GESTUREFLOW_FULL_LAYER_PIO_PROBE[%02d] = 0x%08X} $i [rd32 [expr {$probe_base + $i * 4}]]]
}
if {$final != 0x600D600D} { error "GestureFlow full-layer PIO board run failed" }
puts "GESTUREFLOW_FULL_LAYER_PIO_BOARD_PASS"
disconnect
exit
