...
proc dump_hagrid18_diagnostics {} {
  puts "GESTUREFLOW_HAGRID18_DIAGNOSTICS_BEGIN"
  ...
  puts "GESTUREFLOW_HAGRID18_DIAGNOSTICS_END"
}
...
set bit_path [file join $project_root logs gestureflow_hagrid18_7020.bit]
...
set elf_path [file join $project_root vitis axi_gpio_hagrid18 Debug gestureflow_hagrid18.elf]
...
puts "GESTUREFLOW_HAGRID18_SELECTED_BIT = $bit_path"
puts "GESTUREFLOW_HAGRID18_SELECTED_XSA = $xsa_path"
...
puts [format {GESTUREFLOW_HAGRID18_FINAL_RESULT = 0x%08X} $final]
for {set i 0} {$i < 134} {incr i} {
  puts [format {GESTUREFLOW_HAGRID18_PROBE[%02d] = 0x%08X} $i [rd32 [expr {$probe_base + $i * 4}]]]
}
if {$final != 0x600D600D} {
  dump_hagrid18_diagnostics
  error "GestureFlow HaGRID-18 board run failed"
}
puts "GESTUREFLOW_HAGRID18_BOARD_PASS"
disconnect; exit
