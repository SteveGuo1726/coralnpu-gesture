...
//
// Bounded HP0 DDR reader for one camera-domain RGB frame. It owns one AXI4
// read burst at a time and repacks little-endian 64-bit beats into RGB888
// pixels. RGB is recentered to q=u-128 before it reaches the convolution.
`timescale 1ns/1ps
module gestureflow_hp0_rgb_loader (
  input logic clk, input logic rst_n,
  input logic start, input logic clear,
  input logic [31:0] source_addr,
  input logic [15:0] byte_count,
  input logic [13:0] pixel_count,
  output logic busy, output logic done, output logic fault,
  output logic frame_start, output logic pixel_valid,
  input logic pixel_ready, output logic signed [2:0][7:0] pixel_rgb,
  output logic [13:0] pixels_emitted, output logic [15:0] bytes_read,

  output logic [31:0] m_axi_araddr, output logic [5:0] m_axi_arid,
  ...
  output logic [3:0] m_axi_arqos, output logic [3:0] m_axi_arregion,
  output logic m_axi_arvalid, input wire m_axi_arready,
  input wire [5:0] m_axi_rid, input wire [63:0] m_axi_rdata,
  input wire [1:0] m_axi_rresp, input wire m_axi_rlast,
  input wire m_axi_rvalid, output logic m_axi_rready
);
  ...
  function automatic [4:0] choose_burst_beats(
    input logic [15:0] remaining, input logic [11:0] low_addr
  );
    integer requested;
  ...
  assign pixel_valid = (state != IDLE) && stream_started && (fifo_bytes >= 3);
  assign pixel_fire = pixel_valid && pixel_ready;
  assign pixel_rgb[0] = {~byte_fifo[7], byte_fifo[6:0]};
  ...
  always_comb begin
    ...
    // Avoid simultaneous shift/append; the small FIFO is still enough to
    // hide HP0 response latency because the present MAC consumes slowly.
    m_axi_rready = (state == RECEIVE_R) && (fifo_bytes <= 8) && !pixel_fire;
  end

  always_ff @(posedge clk) begin
    if (!rst_n) begin
      state <= IDLE; next_addr <= '0; bytes_remaining <= '0;
      beats_remaining <= '0; byte_fifo <= '0; fifo_bytes <= '0;
      stream_started <= 1'b0; done <= 1'b0; fault <= 1'b0;
      frame_start <= 1'b0; pixels_emitted <= '0; bytes_read <= '0;
    end else begin
      frame_start <= 1'b0;
      if (clear) begin
        state <= IDLE; done <= 1'b0; fault <= 1'b0; fifo_bytes <= '0;
        stream_started <= 1'b0; pixels_emitted <= '0; bytes_read <= '0;
        bytes_remaining <= '0; beats_remaining <= '0;
      end else begin
        if (!stream_started && state != IDLE && fifo_bytes >= 3) begin
          frame_start <= 1'b1;
          stream_started <= 1'b1;
        end
        if (pixel_fire) begin
          byte_fifo <= byte_fifo >> 24;
        ...
        case (state)
          IDLE: if (start) begin
            done <= 1'b0; fault <= 1'b0; fifo_bytes <= '0; byte_fifo <= '0;
            stream_started <= 1'b0; pixels_emitted <= '0; bytes_read <= '0;
            if ((source_addr[2:0] != 0) || (byte_count == 0) ||
                (byte_count[2:0] != 0) || (byte_count != pixel_count * 3)) begin
              fault <= 1'b1;
            end else begin
