...
# Weight-residency protocol regression: key commit -> cold miss -> warm hit ->
# post-hit weight write rejection.
root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
build="${TMPDIR:-/tmp}/gestureflow_weight_residency_verilator"
rm -rf "$build"
VERILATOR_ROOT=/home/steveguo/verilator /home/steveguo/verilator/bin/verilator_bin --binary --timing --sv -Wno-WIDTHTRUNC -Wno-WIDTHEXPAND \
  --top-module tb_gestureflow_weight_residency --Mdir "$build" -I"$root/tests" \
  ...
  "$root"/tests/tb_gestureflow_weight_residency.sv
timeout 180s "$build/Vtb_gestureflow_weight_residency"
