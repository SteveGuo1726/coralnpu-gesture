...
# Bounded top-level mode-4 HP0 regression. This covers the shared read mux,
# not only the standalone GAP/FC engine.
root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
build="${TMPDIR:-/tmp}/gestureflow_layer_chain_hp0_postprocess_verilator"
rm -rf "$build"
VERILATOR_ROOT=/home/steveguo/verilator /home/steveguo/verilator/bin/verilator_bin --binary --timing --sv -Wno-fatal \
  --top-module tb_gestureflow_layer_chain_hp0_postprocess --Mdir "$build" \
  ...
  "$root/tests/tb_gestureflow_layer_chain_hp0_postprocess.sv"
timeout 180s "$build/Vtb_gestureflow_layer_chain_hp0_postprocess"
