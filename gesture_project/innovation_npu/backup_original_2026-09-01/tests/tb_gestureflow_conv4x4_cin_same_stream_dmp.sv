...
// Small but complete DMP streaming-convolution proof.  The reference model
// computes the raw SAME convolution in 64-bit integer arithmetic; the DUT
// walks the same 3x3 windows through an 8-input-lane DMP tile.  Every output
// vector is compared against the reference, not just a hash or a few probes.
module tb_gestureflow_conv4x4_cin_same_stream_dmp;
  localparam int W = 5;
  localparam int H = 5;
  localparam int C = 8;
  localparam int OC = 4;
  ...
  gestureflow_conv4x4_cin_same_stream_dmp #(
    ...
    .INPUT_CHANNELS(C), .OUT_LANES(OC), .KERNEL_SIZE(K)
  ) dut (
  ...
  task automatic load_weight(input int pair, input int tap);
  ...
  initial begin
    ...
    if ((output_count != H*W) || !frame_input_done_seen || protocol_error || errors != 0) begin
      $fatal(1, "DMP conv failed outputs=%0d done=%b fault=%b errors=%0d",
             output_count, frame_input_done_seen, protocol_error, errors);
    end
    $display("GESTUREFLOW_CONV4X4_CIN_SAME_STREAM_DMP_PASS outputs=%0d", output_count);
    $finish;
  end
endmodule
