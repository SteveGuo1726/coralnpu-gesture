...
`timescale 1ns/1ps
module tb_gestureflow_conv4x4_cin48_tile0_real;
  `include "generated_gestureflow_real_conv4x4_cin48_layer.svh"
  ...
  logic pixel_valid = 0;
  logic pixel_ready;
  logic signed [GF_CIN48_INPUT_CHANNELS-1:0][7:0] pixel_data;
  logic signed [GF_CIN48_INPUT_CHANNELS-1:0][7:0] input_zero_point;
  ...
  logic [$clog2(GF_CIN48_INPUT_CHANNELS/4)-1:0] weight_write_ic_group;
  ...
  gestureflow_conv4x4_cin_same_stream #(
    .IMAGE_WIDTH(GF_CIN48_WIDTH), .IMAGE_HEIGHT(GF_CIN48_HEIGHT),
    .INPUT_CHANNELS(GF_CIN48_INPUT_CHANNELS), .OUT_LANES(GF_TILE_LANES)
  ) stream (
    .clk(clk), .rst_n(rst_n),
    .image_width(16'(GF_CIN48_WIDTH)), .image_height(16'(GF_CIN48_HEIGHT)),
    .pointwise_mode(1'b0),
    ...
    .input_group_count(5'(GF_CIN48_INPUT_CHANNELS/4)), .input_lane_enable(4'hf),
  ...
  always_ff @(posedge clk) begin
    ...
    if (quant_valid && quant_ready) begin
      ...
      for (int lane = 0; lane < GF_TILE_LANES; lane++) begin
        if ($signed(quant_data[lane]) !== gf_cin48_output_q[
          ((row * GF_CIN48_WIDTH + column) * GF_CIN48_LANES) + lane
        ]) begin
          $fatal(1, "cin48 tile0 mismatch y=%0d x=%0d lane=%0d got=%0d expected=%0d",
            row, column, lane, $signed(quant_data[lane]),
            gf_cin48_output_q[((row * GF_CIN48_WIDTH + column) * GF_CIN48_LANES) + lane]);
        end
        output_hash = (output_hash ^ {24'd0, quant_data[lane]}) * 32'h01000193;
      end
      for (int probe = 0; probe < GF_CIN48_PROBE_COUNT; probe++) begin
        if ((row == int'(gf_cin48_probe_y[probe])) && (column == int'(gf_cin48_probe_x[probe]))) begin
          for (int lane = 0; lane < GF_TILE_LANES; lane++) begin
            if ($signed(quant_data[lane]) !== gf_cin48_probe_quant[probe][lane]) begin
              $fatal(1, "cin48 probe mismatch y=%0d x=%0d lane=%0d got=%0d expected=%0d",
                row, column, lane, $signed(quant_data[lane]), gf_cin48_probe_quant[probe][lane]);
            end
          end
  ...
  initial begin
    pixel_data = '0;
    input_zero_point = {GF_CIN48_INPUT_CHANNELS{-8'sd128}};
    weight_write_oc = '0;
    weight_write_tap = '0;
    ...
    quant_column = '0;
    for (int lane = 0; lane < GF_TILE_LANES; lane++) begin
      bias[lane] = gf_cin48_folded_bias[lane];
      requant_multiplier[lane] = gf_cin48_multiplier[lane];
      requant_right_shift[lane] = gf_cin48_right_shift[lane];
    end
    ...
    for (int oc = 0; oc < GF_TILE_LANES; oc++) begin
      for (int tap = 0; tap < 16; tap++) begin
        for (int group = 0; group < GF_CIN48_INPUT_CHANNELS / 4; group++) begin
          @(negedge clk);
          weight_write_valid = 1;
          weight_write_oc = $clog2(GF_TILE_LANES)'(oc);
          weight_write_tap = 4'(tap);
          weight_write_ic_group = $clog2(GF_CIN48_INPUT_CHANNELS/4)'(group);
          for (int lane = 0; lane < 4; lane++) begin
            weight_write_data[lane] = gf_cin48_weights[
              (oc * 16 * GF_CIN48_INPUT_CHANNELS) + (tap * GF_CIN48_INPUT_CHANNELS) + (group * 4) + lane
            ];
          end
          @(negedge clk);
    ...
    @(negedge clk);
    frame_start = 0;
    for (int row = 0; row < GF_CIN48_HEIGHT; row++) begin
      for (int column = 0; column < GF_CIN48_WIDTH; column++) begin
        while (!pixel_ready) @(negedge clk);
        for (int channel = 0; channel < GF_CIN48_INPUT_CHANNELS; channel++) begin
          pixel_data[channel] = gf_cin48_input_q[
            ((row * GF_CIN48_WIDTH + column) * GF_CIN48_INPUT_CHANNELS) + channel
          ];
        end
        pixel_valid = 1;
        @(negedge clk);
    ...
    for (int watchdog = 0; watchdog < 1200000 && output_count < GF_CIN48_WIDTH * GF_CIN48_HEIGHT; watchdog++) begin
      @(negedge clk);
    end
    @(negedge clk);
    if (raw_fault || quant_fault || !frame_input_done_seen || probe_hits != GF_CIN48_PROBE_COUNT ||
        output_count != GF_CIN48_WIDTH * GF_CIN48_HEIGHT) begin
      $fatal(1, "cin48 tile0 failed outputs=%0d probes=%0d done=%b raw_fault=%b quant_fault=%b hash=%08x",
        output_count, probe_hits, frame_input_done_seen, raw_fault, quant_fault, output_hash);
    end
    $display("GESTUREFLOW_CONV4X4_CIN48_TILE0_REAL_PASS outputs=%0d probes=%0d hash=%08x",
