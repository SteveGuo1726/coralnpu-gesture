...
proc package_ip {src_dir project_root} {
  set root [file join $project_root ip_repo gestureflow_body2_hp0_axil_1.0]
  set pack [file join $project_root .tmp_gestureflow_body2_hp0_pack]
  file delete -force $root; file delete -force $pack; file mkdir [file dirname $root]
  create_project -force gestureflow_body2_hp0_pack $pack -part xc7z020clg400-2
  foreach src {gestureflow_line_delay_bank.sv gestureflow_line_window.sv gestureflow_same4x4_cin_window.sv gestureflow_weight_bank.sv gestureflow_mac_tile.sv gestureflow_conv4x4_cin_same_stream.sv gestureflow_requant_relu.sv gestureflow_hp0_tensor_loader.sv gestureflow_body2_hp0_axil.sv} {
  ...
  set_property top gestureflow_body2_hp0_axil [current_fileset]
  update_compile_order -fileset sources_1
  ipx::package_project -root_dir $root -vendor user.org -library user -taxonomy /UserIP -import_files -force
  set core [ipx::current_core]
  set_property name gestureflow_body2_hp0_axil $core
  set_property display_name {GestureFlow body-2 HP0 baseline} $core
  set_property description {Project-local 96x96x16 TFLite body convolution with PS DDR ingress over HP0.} $core
  ipx::save_core $core
...
create_bd_cell -type ip -vlnv user.org:user:gestureflow_body2_hp0_axil:1.0 gestureflow_0
...
connect_bd_net [get_bd_pins processing_system7_0/FCLK_CLK0] \
  [get_bd_pins gestureflow_0/aclk] [get_bd_pins processing_system7_0/S_AXI_HP0_ACLK] [get_bd_pins axi_smc/aclk]
...
if {$control_segment eq ""} { error "Body-2 IP did not expose AXI-Lite" }
...
report_utilization -hierarchical -file [file join $log_dir gestureflow_body2_hp0_7020_utilization_impl.rpt]
report_timing_summary -file [file join $log_dir gestureflow_body2_hp0_7020_timing_impl.rpt]
write_hw_platform -fixed -include_bit -force -file [file join $log_dir gestureflow_body2_hp0_7020.xsa]
puts "GESTUREFLOW_BODY2_HP0_7020_BITSTREAM_PASS project=$project_root"
close_project; exit
