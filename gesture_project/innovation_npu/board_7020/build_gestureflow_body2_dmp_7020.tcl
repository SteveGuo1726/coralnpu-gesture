...
proc package_ip {src_dir project_root} {
  set root [file join $project_root ip_repo gestureflow_body2_dmp_hp0_axil_1.0]
  set pack [file join $project_root .tmp_gestureflow_body2_dmp_pack]
  file delete -force $root; file delete -force $pack; file mkdir [file dirname $root]
  create_project -force gestureflow_body2_dmp_pack $pack -part xc7z020clg400-2
  foreach src {gestureflow_line_delay_bank.sv gestureflow_line_window.sv gestureflow_line_delay_vector_bank.sv gestureflow_line_window_vector.sv gestureflow_same4x4_cin_window.sv gestureflow_weight_bank.sv gestureflow_mac_tile_dmp.sv gestureflow_conv4x4_cin_same_stream_dmp.sv gestureflow_requant_relu.sv gestureflow_hp0_tensor_loader.sv gestureflow_body2_dmp_hp0_axil.sv} {
    set path [file join $src_dir $src]
    if {![file exists $path]} { error "Missing GestureFlow DMP source: $path" }
    add_files -norecurse $path; set_property file_type SystemVerilog [get_files $path]
  }
  set_property top gestureflow_body2_dmp_hp0_axil [current_fileset]
  update_compile_order -fileset sources_1
  ipx::package_project -root_dir $root -vendor user.org -library user -taxonomy /UserIP -import_files -force
  set core [ipx::current_core]
  set_property name gestureflow_body2_dmp_hp0_axil $core
  set_property display_name {GestureFlow body-2 DMP HP0} $core
  set_property description {Project-local 96x96x16 TFLite body convolution on the DMP 8-lane dual-multiply engine.} $core
  ipx::save_core $core
...
create_bd_cell -type ip -vlnv user.org:user:gestureflow_body2_dmp_hp0_axil:1.0 gestureflow_0
...
if {$control_segment eq ""} { error "Body-2 DMP IP did not expose AXI-Lite" }
...
report_utilization -hierarchical -file [file join $log_dir gestureflow_body2_dmp_7020_utilization_impl.rpt]
report_timing_summary -file [file join $log_dir gestureflow_body2_dmp_7020_timing_impl.rpt]
write_hw_platform -fixed -include_bit -force -file [file join $log_dir gestureflow_body2_dmp_7020.xsa]
file copy -force [file join $project_root axi_gpio.runs impl_1 system_wrapper.bit] [file join $log_dir gestureflow_body2_dmp_7020.bit]
puts "GESTUREFLOW_BODY2_DMP_7020_BITSTREAM_PASS project=$project_root"
close_project; exit
