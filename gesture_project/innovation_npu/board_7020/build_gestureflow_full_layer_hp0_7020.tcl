...
# Independent 7020 hardware baseline: PS DDR -> HP0 -> full GestureFlow layer.
proc package_ip {src_dir project_root} {
  set root [file join $project_root ip_repo gestureflow_full_layer_hp0_axil_1.0]
  ...
  set_property top gestureflow_full_layer_hp0_axil [current_fileset]
...
create_bd_cell -type ip -vlnv user.org:user:gestureflow_full_layer_hp0_axil:1.0 gestureflow_0
...
# Retain a low-risk 25 MHz initial board clock. The tile has its own later
# timing-closure path; this first DDR baseline must be bit-exact and robust.
...
if {$control_segment eq ""} { error "HP0 full-layer IP did not expose AXI-Lite" }
assign_bd_address -offset 0x43C00000 -range 0x00100000 \
  -target_address_space [get_bd_addr_spaces processing_system7_0/Data] $control_segment
...
# The copied tutorial has an imported wrapper which still instantiates its
# former CoreMini IP even after its BD cell has been deleted. Remove it before
# importing the wrapper generated from this HP0-only block design.
...
report_utilization -hierarchical -file [file join $log_dir gestureflow_full_hp0_7020_utilization_impl.rpt]
report_timing_summary -file [file join $log_dir gestureflow_full_hp0_7020_timing_impl.rpt]
write_hw_platform -fixed -include_bit -force -file [file join $log_dir gestureflow_full_hp0_7020.xsa]
puts "GESTUREFLOW_FULL_HP0_7020_BITSTREAM_PASS project=$project_root"
close_project; exit
