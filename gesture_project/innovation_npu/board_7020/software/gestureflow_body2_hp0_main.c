...
/* Real TFLite body convolution: signed INT8 tensor in PS DDR through HP0. */
#include <stdint.h>
#include "sleep.h"
#include "xil_cache.h"
...
#include "xil_types.h"
#include "xtime_l.h"
#include "gestureflow_real_conv4x4_body2_layer.h"
...
static int8_t gf_hp0_tensor[GF_FULL_IMAGE_WIDTH * GF_FULL_IMAGE_HEIGHT * GF_FULL_INPUT_CHANNELS]
...
static void terminal_failure(u32 code, u32 observed)
{
    ...
    xil_printf("GESTUREFLOW_BODY2_HP0_FAIL code=%08lx observed=%08lx\r\n",
...
int main(void)
{
    u32 oc, tap, group, lane, index, status, dma_status, cycles, input_count, output_count, hash;
    XTime t0, t1;
    ...
    if (Xil_In32(GF_BASE + GF_VERSION) != 0x00030001U) terminal_failure(0x3002U, Xil_In32(GF_BASE + GF_VERSION));
    if ((((u32)(UINTPTR)gf_hp0_tensor) & 7U) != 0U) terminal_failure(0x3003U, (u32)(UINTPTR)gf_hp0_tensor);

    stage = 0x20U;
    for (index = 0U; index < image_bytes; ++index) gf_hp0_tensor[index] = gf_full_layer_input[index];
    ...
    for (oc = 0U; oc < GF_FULL_OUTPUT_LANES; ++oc) {
        ...
        for (tap = 0U; tap < 16U; ++tap) {
            for (group = 0U; group < GF_FULL_INPUT_CHANNELS / 4U; ++group) {
                u32 packed = 0U;
                for (lane = 0U; lane < 4U; ++lane) {
                    u32 wi = (oc * 16U * GF_FULL_INPUT_CHANNELS) + (tap * GF_FULL_INPUT_CHANNELS) + (group * 4U) + lane;
                    packed |= (u32)(uint8_t)gf_full_weights[wi] << (lane * 8U);
                }
    ...
    if ((status & (GF_FAULT_BIT | GF_LAYER_FAULT_BIT)) || (dma_status & GF_DMA_FAULT_BIT) ||
        ...
        hash != GF_FULL_OUTPUT_FNV1A || cycles == 0U) terminal_failure(0x3004U, hash);
    store_probe(0U, GF_RESULT_PASS);
    xil_printf("GESTUREFLOW_BODY2_HP0_BOARD_PASS cycles=%lu hash=%08lx dma=%08lx\r\n",
