...
set project_root "E:/coralnpu_vivado/projects/gestureflow_body2_hp0_7020_v2"
set bit_path [file join $project_root axi_gpio.runs impl_1 system_wrapper.bit]
set xsa_path [file join $project_root logs gestureflow_body2_hp0_7020.xsa]
set ps7_init_path [file join $project_root axi_gpio.srcs sources_1 bd system ip system_processing_system7_0_0 ps7_init.tcl]
set elf_path [file join $project_root vitis axi_gpio Debug gestureflow_body2_hp0.elf]
...
puts [format {GESTUREFLOW_BODY2_HP0_FINAL_RESULT = 0x%08X} $final]
for {set i 0} {$i < 15} {incr i} {
  puts [format {GESTUREFLOW_BODY2_HP0_PROBE[%02d] = 0x%08X} $i [rd32 [expr {$probe_base + $i * 4}]]]
}
if {$final != 0x600D600D} { error "GestureFlow body2 HP0 board run failed" }
puts "GESTUREFLOW_BODY2_HP0_BOARD_PASS"
disconnect; exit
