...
set project_root "E:/coralnpu_vivado/projects/gestureflow_body2_dmp_7020_v1"
set bit_path [file join $project_root logs gestureflow_body2_dmp_7020.bit]
set xsa_path [file join $project_root logs gestureflow_body2_dmp_7020.xsa]
set ps7_init_path [file join $project_root axi_gpio.srcs sources_1 bd system ip system_processing_system7_0_0 ps7_init.tcl]
set elf_path [file join $project_root vitis axi_gpio Debug gestureflow_body2_dmp.elf]
...
puts [format {GESTUREFLOW_BODY2_DMP_FINAL_RESULT = 0x%08X} $final]
for {set i 0} {$i < 15} {incr i} {
  puts [format {GESTUREFLOW_BODY2_DMP_PROBE[%02d] = 0x%08X} $i [rd32 [expr {$probe_base + $i * 4}]]]
}
if {$final != 0x600D600D} { error "GestureFlow body2 DMP board run failed" }
puts "GESTUREFLOW_BODY2_DMP_BOARD_PASS"
disconnect; exit
