...
# Compatibility entry kept for older command lines.
# Row handoff counters are project-side registers in this board shell, not
# official CoreCSR registers from the clean Google CoralNPU RTL.
...
connect
puts "ROWHANDOFF_PROJECT_EVENT_TRACE_TARGETS_BEGIN"
puts [targets]
puts "ROWHANDOFF_PROJECT_EVENT_TRACE_TARGETS_END"
...
puts "ROWHANDOFF_PROJECT_EVENT_TRACE_RESET_AND_SELECT_MODE"
write32 [expr {$base_addr + 0x08}] 0x00000015
after 20
write32 [expr {$base_addr + 0x08}] 0x00000011
after 20

set event_addr [expr {$base_addr + 0x40}]
foreach event_word $events {
  write32 $event_addr $event_word
}
after 20
...
if {($event_status & 0x0000000A) != 0x0000000A} {
  error [format "Project event status bits invalid: 0x%08X" $event_status]
}

puts "ROWHANDOFF_PROJECT_EVENT_TRACE_COUNTS_PASS"
exit
