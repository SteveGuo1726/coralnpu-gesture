      ...
      if (is_setvl[i]) begin
        ...
        if (REDUCE_LMUL && !inst_config_state[i+1].vill) begin
          ...
          unique case (inst_config_state[i+1].sew)
            ...
            `ifdef VLEN_64
                // VLEN=64 时不存在 VLEN/128+1 到 VLEN/64 这段可再细分区间，
                // 这里直接落到最小保留档，避免出现 `VL_WIDTH-8` 负索引。
            `else
...
`endif
      ...
      // PROJECT_LOCAL_MOD: the accelerator-only lane1 wrapper injects scalar
      // operands directly through the two register-read slots. For LSU ops we
      // must always propagate rs1 explicitly, otherwise unit-stride DDR access
      // falls back to zero/garbage base addresses.
      unaligned_cmd_data[i].rs1 =
          (inst_q[i].opcode != RVV) ? reg_read_data_i[2*i] :
          inst_q[i].bits[7] ?
