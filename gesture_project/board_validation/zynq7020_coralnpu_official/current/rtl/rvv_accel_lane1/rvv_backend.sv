...
`ifdef PROJECT_LOCAL_CONV_ACCEL_ONLY
    // PROJECT_LOCAL_MOD: conv-focused cut disables PMTRDT issue on this lane1
    // accelerator build. Unsupported PMTRDT uops are back-pressured instead
    // of being silently dropped.
    assign pmtrdt_rs_almost_full = '1;
    assign fifo_almost_empty_rs2pmtrdt = '1;
    assign uop_rs2pmtrdt = '{default:'0};
    assign pop_pmtrdt2rs = '0;
    assign rs_ready_pmtrdt2dp = '0;
`else
...
`ifdef PROJECT_LOCAL_CONV_ACCEL_ONLY
    // PROJECT_LOCAL_MOD: conv-focused cut disables DIV issue on this lane1
    // accelerator build. Unsupported DIV uops are back-pressured instead of
    // being silently dropped.
    assign div_rs_almost_full = '1;
    assign fifo_almost_empty_rs2div = '1;
    assign uop_rs2div = '{default:'0};
    assign pop_div2rs = '0;
    assign rs_ready_div2dp = '0;
`else
    // DIV RS
    multi_fifo #(
        .T              (DIV_RS_t),
...
`ifdef PROJECT_LOCAL_CONV_ACCEL_ONLY
    ...
    assign res_valid_pmtrdt = '0;
    assign res_pmtrdt = '{default:'0};
`else
    // PMTRDT
    rvv_backend_pmtrdt #(
    ) u_pmtrdt (
...
`ifdef PROJECT_LOCAL_CONV_ACCEL_ONLY
    assign res_valid_div = '0;
    assign res_div = '{default:'0};
`else
