...
module coralnpu_stage3b_tensor_engine (
  ...
  // ports; ST_WRITE owns output writes. With mem_kind=5 in IDLE, mem_rdata is
  // valid two clocks after the PS has held an output address. The extra
  ...
  always @(posedge clk) begin
    ...
    if (read_req_q) begin
      mem_rdata <= read_addr_q[0] ? output_mem[read_addr_q[13:1]][63:32] :
                                   output_mem[read_addr_q[13:1]][31:0];
    end
  end
  ...
  always @(posedge clk) begin
    if (!rstn) begin
      ...
      read_req_q <= (state == ST_IDLE) && (mem_kind == 3'd5);
