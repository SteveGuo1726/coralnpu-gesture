...
// ---internal signal definition--------------------------------------
  ...
  `else 
    ...
    if (`NUM_PU == 5) begin : gen_project_local_lane1_arb
      logic [1:0] req_tail;
      logic [1:0] grant_tail;

      assign req_tail = req[4:3];

      arb_round_robin #(.REQ_NUM(2))
      arb_tail (.grant(grant_tail), .req(req_tail), .clk(clk), .rst_n(rst_n));
      ...
      always_comb begin
        ...
        grant[1] = req[1];
        grant[2] = req[2];
        grant[3] = grant_tail[0];
        grant[4] = grant_tail[1];

        result_valid[0] = req[0];
        result_valid[1] = req[1];
        result_valid[2] = req[2];
        result_valid[3] = |grant_tail;

        result[0] = item[0];
        result[1] = item[1];
        result[2] = item[2];
        result[3] = grant_tail[0] ? item[3] : item[4];
      end
    end else begin : gen_original_no_float_arb
      logic [1:0] req_mac;
      logic [1:0] grant_mac;
