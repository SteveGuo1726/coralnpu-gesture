...
// PROJECT_LOCAL_MOD: vmacc.vx follows vector-scalar semantics.  The scalar
// must feed every e32 lane; zero-extending it to VLEN only drove lane zero
// when this 64-bit build issued VL=2.
wire [`VLEN-1:0] mac_uop_rs1_eew32_broadcast =
    {(`VLEN/`XLEN){mac_uop_rs1_data}};
`ifdef TB_SUPPORT
  ...
  case ({rs2mac_uop_valid,mac_uop_funct3}) 
    ...
    {1'b1,OPMVX} : begin
      ...
      case (mac_uop_funct6) 
        VMACC : begin
          mac_src2           = mac_uop_vs2_data;
          mac_src1           = (mac_top_vs_eew == EEW32) ?
                               mac_uop_rs1_eew32_broadcast :
                               {(`VLEN-`XLEN)'('b0),mac_uop_rs1_data};
          mac_addsrc         = mac_uop_vs3_data;
          mac_src2_is_signed = 1'b1;
          mac_src1_is_signed = 1'b1;
          mac_is_widen       = 1'b0;
