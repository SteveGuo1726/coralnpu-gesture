...
//
  ...
  always_comb begin
    for(int i=0;i<`NUM_DE_UOP;i++) begin: PSHROB_VLD
      case(uop_exe_unit)
      `ifdef ZVT_ON
        // WORKTREE-EXPERIMENT: most VME-side tile operations do not need an RVV
        // ROB retire record, but vtmv.v.t must still retire back to the scalar
        // side because the front-end tracks it as a vector-writing instruction.
        // Without a ROB record here, ZVT can produce an internal tile->vector
        // result while the scalar retirement buffer never observes a matching
        // rd_rob2rt completion pulse.
        VME: begin
          if ((inst_funct3==OPMVX) && (inst_funct6==VWRXUNARY0) && (vs2_opcode==VTMVVT))
            pshrob_valid[i] = last_uop_valid[i];
          else
            pshrob_valid[i] = 1'b0;
        end
      `endif
