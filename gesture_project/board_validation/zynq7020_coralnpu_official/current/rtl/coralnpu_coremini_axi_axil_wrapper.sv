// PROJECT_LOCAL_MOD: 7020 board shell wrapper for running upstream CoreMiniAxi
// through a 32-bit AXI-Lite control path from the Zynq PS.
`timescale 1ns / 1ps

module coralnpu_coremini_axi_axil_wrapper (
  ...
  localparam logic [31:0] LOCAL_MAGIC_ADDR   = 32'h0004_0000;
  localparam logic [31:0] LOCAL_STATUS_ADDR  = 32'h0004_0004;
  localparam logic [31:0] LOCAL_VERSION_ADDR = 32'h0004_0008;
  localparam logic [31:0] LOCAL_MAGIC_VALUE  = 32'h4352_4C4E;  // "CRLN"
  localparam logic [31:0] LOCAL_VERSION_VALUE = 32'h0001_0000;
  ...
  logic        wr_addr_seen;
  logic        wr_data_seen;
  logic        wr_core_aw_sent;
  logic        wr_core_w_sent;

  logic [31:0] rd_addr_reg;
  logic        rd_core_ar_sent;

  logic [5:0]  core_slave_b_id;
  logic [1:0]  core_slave_b_resp;
  logic        core_slave_b_valid;
  logic [127:0] core_slave_r_data;
  logic [5:0]   core_slave_r_id;
  logic [1:0]   core_slave_r_resp;
  logic         core_slave_r_last;
  logic         core_slave_r_valid;
  ...
  wire [127:0] wr_shifted_data = {{96{1'b0}}, wr_data_reg} << (wr_lane * 32);
  wire [15:0] wr_shifted_strb = {{12{1'b0}}, wr_strb_reg} << (wr_lane * 4);

  wire [1:0] rd_lane = rd_addr_reg[3:2];
  wire [31:0] rd_aligned_addr = {12'd0, rd_addr_reg[19:4], 4'b0000};
  wire [31:0] rd_core_word =
      core_slave_r_data[(rd_lane * 32) +: 32];

  wire wr_is_local = (wr_offset_addr == LOCAL_MAGIC_ADDR) ||
                     (wr_offset_addr == LOCAL_STATUS_ADDR) ||
                     (wr_offset_addr == LOCAL_VERSION_ADDR);
  wire rd_is_local = (rd_offset_addr == LOCAL_MAGIC_ADDR) ||
                     (rd_offset_addr == LOCAL_STATUS_ADDR) ||
                     (rd_offset_addr == LOCAL_VERSION_ADDR);

  logic        core_slave_aw_ready;
  logic        core_slave_w_ready;
  logic        core_slave_ar_ready;
  logic        core_slave_b_ready;
  logic        core_slave_r_ready;
  logic        core_slave_aw_valid;
  logic        core_slave_w_valid;
  logic        core_slave_ar_valid;
        ...
        RD_IDLE: begin
          if (s_axi_arvalid && s_axi_arready) begin
            ...
            if ((ar_offset_addr == LOCAL_MAGIC_ADDR) ||
              ...
              case (ar_offset_addr)
                LOCAL_MAGIC_ADDR:   s_axi_rdata <= LOCAL_MAGIC_VALUE;
                LOCAL_STATUS_ADDR:  s_axi_rdata <= {29'd0, core_fault, core_wfi, core_halted};
                LOCAL_VERSION_ADDR: s_axi_rdata <= LOCAL_VERSION_VALUE;
