...
## ⚠️ 读文档的规矩（先看这条，否则会被过时文档带偏）
仓库有 **56 个 `.md`**，绝大部分是**历史快照**。判定规则：

1. **只有 `docs/会话交接_最高优先级_2026-07-11.md` 的「编号章节」是权威且一直在记录的**
   （147 号往下，倒序）。**它尾部的 `## 当前有效结论` / `## 下一轮执行入口` 是
   `2026-08-02` 快照**（仍写 RepVGG+landmark 94.62%、3×3 保底 79.24%、双后端、VME 未打通），
   **标题叫"当前"但完全不是当前** —— 已加横幅。
2. **当前现状全景** → `docs/项目完整上下文_Windows工作区版_2026-09-12.md`。
3. **动手前先看** → `docs/ZCU104_上板操作卡_2026-09-12.md` + `board_zcu104/plnx/PITFALLS.md`。
4. **已确认过时的入口文档，不要拿它判断主线**：
   `docs/工程文件索引.md`、`docs/仓库共享版_活跃代码与文件说明.md`、
   `innovation_npu/README.md`（停在 v0 的 16×4）、`qwen-analysis.md`（第三方原稿，已被复核降级）。
   这四类已加"⚠️ 已过时"横幅。
5. **例外**：`7020帧率优化完整方案…_2026-08-21.md` 的性能数字已过时，
   **但 §1.0 与 §1.4 的六条架构硬门禁仍然有效**，是项目的价值观来源，必须读。
6. **权威文档里的具体事实也要用源码复核**：曾经 `gesture_project/README.md` 的模型图
   就漏了一层（`body2` 16→16）。**层数以 `board_7020/software/` 的逐层权重头文件为准**
   （实际 7 个：full 3→16 / body2 16→16 / conv2a / conv2b / conv3a / conv3b / head1x1）。
7. 全量判定表见 `docs/文档可信度地图_2026-09-12.md`。
